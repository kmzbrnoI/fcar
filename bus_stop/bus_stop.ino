...
void setup()
{
    ...
    junction_a = new Junction(9, 55, 20);
    semaphore_a = new Semaphore(11, 0, 70);
...
void manage_bus_stop(HallProbe *drive_probe, HallProbe *type_probe, Junction *junction,
                     Semaphore *semaphore)
{
    ...
    if (switch_delta < SWITCH_PASSAGE_TIME && junction->getDirection() == JDirection::plus
        && semaphore->getSignal() == SSignal::green) {
        junction->to_minus();
        semaphore->signal_red();
    } else if (switch_delta > SWITCH_PASSAGE_TIME
               && (junction->getDirection() == JDirection::minus)) {
        junction->to_plus();
    }
}
